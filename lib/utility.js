...
exports.getLinksList = function(req, res) {
  exports.pullUserId(req.session.user, function(usernameResult) {
    var user_id = usernameResult[0].id;
    Links.reset().fetch().then(function(links) {
      var allLinks = links.models;
      var userLinks = [];
      _(allLinks).each(function(model) {
        if (model.attributes.user_id === user_id) {
          userLinks.push(model);
        }
      });
      res.send(200, userLinks);
    });
  });
};

exports.pullUserId = function(username, cb) {
  db.knex('users').where({username: username}).then(cb);
};
...
exports.createShortLink = function(req, res) {
  ...
  exports.pullUserId(username, function(usernameResult) {
    console.log(usernameResult, usernameResult[0].id);
    var user_id = usernameResult[0].id;
